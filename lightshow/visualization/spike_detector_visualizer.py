...
class SpikeDetectorVisualizer:
    def __init__(
        self,
        SpikeDetector: effects.SpikeDetector,
        ax: Axes,
        ...
        self.marker_types = {
            "beat": {
                "indices": [],
                "energies": [],
                "scatter": ax.scatter([], [], color="magenta", label="Beat", zorder=5),
            },
            "break": {
                "indices": [],
                "energies": [],
                "scatter": ax.scatter([], [], color="green", label="Break", zorder=5),
            },
            "drop": {
                "indices": [],
                "energies": [],
                "scatter": ax.scatter([], [], color="orange", label="Drop", zorder=5),
            },
        }

        (self.line_energy,) = ax.plot([], [], lw=2, color="yellow", label="Energy")
    ...
    def _add_marker(self, marker_type, detected, current_energy):
        if detected:
            ...
            if len(self.marker_types[marker_type]["indices"]) > maxlen:
                self.marker_types[marker_type]["indices"] = self.marker_types[
                    marker_type
                ]["indices"][-maxlen:]
                self.marker_types[marker_type]["energies"] = self.marker_types[
                    marker_type
                ]["energies"][-maxlen:]
    ...
    def __call__(
        ...
        if len(self.SpikeDetector.energy_history) < 1:
            return (
                self.line_energy,
                self.line_limit,
                *(mt["scatter"] for mt in self.marker_types.values()),
            )
